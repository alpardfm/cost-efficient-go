...
// StubborneWorker simulates a worker that is slow to respond to cancellation.
...
func main() {
	...
	detector := NewLeakDetector("LeakyServer")
	LeakyServer(100)
	...
	// Safe implementation
	detector2 := NewLeakDetector("SafeServer")
	...
	graceful := GracefulShutdown(500*time.Millisecond, LongRunningWorker)
	if graceful {
	...
	graceful = GracefulShutdown(100*time.Millisecond, StubbornWorker)
	if graceful {
		fmt.Println("✓ Graceful shutdown")
	} else {
		fmt.Println("✗ Forced termination")
